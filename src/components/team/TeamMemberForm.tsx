...
export function TeamMemberForm({ member, onSave, onCancel, isLoading }: TeamMemberFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave({ name, email, phone, default_role: defaultRole, notes, is_active: isActive });
  };
    ...
    <div className="fixed inset-0 bg-ink-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-parchment rounded-lg border border-wood-medium/40 parchment-shadow w-full max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-wood-dark mb-1">Role</label>
            <input
              type="text"
              value={defaultRole}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-wood-dark mb-1">Email</label>
              <input
                type="email"
                value={email}
            ...
            <div>
              <label className="block text-sm font-medium text-wood-dark mb-1">Phone</label>
              <input
                type="tel"
                value={phone}
          ...
          <div>
            <label className="block text-sm font-medium text-wood-dark mb-1">Notes</label>
            <textarea
              value={notes}
          ...
          {member && (
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="is_active" className="text-sm text-wood-dark">Active member</label>
            </div>
          )}
