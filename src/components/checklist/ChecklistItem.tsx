  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm ${isCompleted ? 'line-through text-sepia/60' : 'text-ink-black'}`}>
          {item.title}
        </p>
        {item.description && (
          <p className="text-xs text-sepia/60 mt-0.5">{item.description}</p>
        )}
        <div className="flex items-center gap-3 mt-1.5">
          {assignee && (
            <span className="flex items-center gap-1 text-xs text-sepia/70">
